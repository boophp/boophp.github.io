...
class db extends DCore_BackApp
{
    ...
    {
        global $argc, $argv;
        echo "Usage: php " . $argv[0] . " dbfile=<dbfile> dbname=<dbname> configdb=<configdb>\n";
    }

    protected function checkParameter()
    {
        if (!$this->dbfile || !is_file($this->dbfile) || !$this->dbname ||!$this->configdb)
        {
            $this->printUsage();
            exit;
        }
    ...
    protected function main()
    {
        ...
        if (preg_match_all($pattern, $newscontent, $result))
        {
            foreach ($result[0] as $key => $item)
            {
                ...
                echo "php newtable_singlenode.php configdb={$this->configdb} kind=" . $kind . "  sid=" . $sid . " db=".$this->dbname." sqlfile=$table_name.sql\n";
                //      echo $filecontent."\n";
                echo $table_name . "\n";
            }
        }
